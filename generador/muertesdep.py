...
for i in range(len(dep)):
    ...
    plt.title('\nFallecimientos/día cada 100\'000 HAB en el Dep.:\n'+dep[i]+'\n(último reporte en fuente: '+y[-1]+')\n',fontsize=75,fontproperties=prop,color=tableau20[1])
    for j in range(9):
        if dep[j]!=dep[i]:
            plt.plot(y,mean_muertes[j]/n_dep[j],label=dep[j],linewidth=3,linestyle='-')
        else:
            pass
    plt.plot(y,mean_muertes[i]/n_dep[i],label='Promedio 7 días '+dep[i],linewidth=8,color=tableau20[4],linestyle='-')   
   ...
   # plt.ylim(0,2*np.max(var_mc[i]))  
    ...
    plt.text(0,-1*np.max(mean_muertes[0]/n_dep[0]),"Data source: https://www.udape.gob.bo/index.php?option=com_wrapper&view=wrapper&Itemid=104" 
       "\nAutor: Telegram Bot: @Bolivian_Bot"    
       "\nNota: Curva de fallecimientos/día ajustada a 100 000 hab",fontsize=40,fontproperties=prop,color=tableau20[1]);
    plt.savefig('imagenes/muertes'+dep[i]+'.png');
#     plt.close(fig)
